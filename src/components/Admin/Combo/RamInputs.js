...
export const RamInputs = ({ product, validate }) => {
    ...
    return (
        <Box component="div" sx={{
            ...
            <TextField disabled={!validate} required={validate} id="ram_brand" fullWidth placeholder="Marca de la RAM" label="Marca" defaultValue={product?.combo_data?.ram?.brand} />
            <TextField disabled={!validate} required={validate} id="ram_model" fullWidth placeholder="Modelo de la RAM" label="Modelo" defaultValue={product?.combo_data?.ram?.model} />
            <div>
                <InputLabel>Capacidad</InputLabel>
                <Select disabled={!validate} required={validate} id="ram_capacity" name="ram_capacity" fullWidth value={ramCapacity} onChange={e => setRamCapacity(e.target.value)}>
            ...
            <div>
                ...
                <Select disabled={!validate} required={validate} id="ram_gen" name="ram_gen" fullWidth value={ramGen} onChange={e => setRamGen(e.target.value)}>
                    <MenuItem sx={{ fontSize: 14 }} value={"DDR4"}>DDR4</MenuItem>
                    <MenuItem sx={{ fontSize: 14 }} value={"DDR3"}>DDR3</MenuItem>
                </Select>
            </div>
            <TextField disabled={!validate} required={validate} id="ram_mhz" name="ram_mhz" type="number" fullWidth placeholder="Capacidad en MHz" label="Capacidad en MHz" defaultValue={product?.combo_data?.ram?.mhz} />
            <div>
                <InputLabel>Forma</InputLabel>
                <Select disabled={!validate} required={validate} id="ram_form" name="ram_form" fullWidth value={ramForm} onChange={e => setRamForm(e.target.value)}>
